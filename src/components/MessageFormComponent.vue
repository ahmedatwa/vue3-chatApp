...
const onSubmit = () => {
  if (!isEmpty(messageValue.value) || uploadedFile.value) {
    emit("submit", { text: messageValue.value, file: uploadedFile.value });
  }
  messageValue.value = "";
  removeFile();
};
  ...
  <v-card :elevation="4">
    ...
    <v-sheet>
      ...
      <div class="float-right">
        <v-btn icon="mdi-plus-circle" density="default" color="orange-accent-2"
          @click="open"></v-btn>
        <v-btn icon="mdi-emoticon-happy-outline" density="default" id="emoji-activator"
