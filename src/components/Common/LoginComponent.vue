...
<template>
  <v-card class="mx-auto" width="500" v-if="!isLogged">
    <v-btn @click.stop="toggle = !toggle" prepend-icon="mdi-eye-circle" color="indigo" class="ma-4">Add User</v-btn>
    <v-form id="create-user-form mt-2" v-if="toggle" @submit.prevent="createUser" class="ma-4">
      <v-text-field label="firstname" v-model="form.firstName" type="text" :loading="sessionStore.isLoading">
      </v-text-field>
      <v-text-field label="last name" v-model="form.lastName" type="text" :loading="sessionStore.isLoading">
      </v-text-field>
    ...
    <v-table>
      ...
      <tbody>
        ...
        <tr class="text-end">
          <td colspan="6">
            <v-btn :disabled="!selected" :loading="sessionStore.isLoading" @click="login" icon="mdi-location-enter"
              color="primary">
            </v-btn>
          </td>
        </tr>
